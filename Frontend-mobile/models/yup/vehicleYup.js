...
const vehicleYup = yup.object({
  ...
  modelo: yup.string().max(50, "El modelo del vehículo no debe tener más de 50 caracteres").required("El campo modelo no puede estar vacío").trim(),
  placa: yup
    .string()
    ...
    .nullable()
    .trim(),
  desc: yup.string().max(250, "La descripción no debe tener más de 250 caracteres").required("El campo descripción no puede estar vacío").trim(),
  status: yup.boolean()
});
